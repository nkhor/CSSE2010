...
int8_t advance_snake_head(void) {
	...
	*/
	if (is_snake_at(newHeadPosn)) {
		if (newHeadPosn == get_snake_tail_position()) {
			...
			printf("Snake has run into itself.");
			return COLLISION;
		}
	}
	...
	*/
	if(is_food_at(newHeadPosn)) {
		if (newHeadPosn == return_stored_super()) {
			if(snakeLength <= MAX_SNAKE_SIZE) {
				add_to_score(10);
				return ATE_FOOD;
			} else {
		...
		} else if (newHeadPosn == return_rat_posn()) {
			if(snakeLength <= MAX_SNAKE_SIZE) {
				add_to_score(5);
				return ATE_FOOD;
			} else {
		...
		} else { //there's food but not super food
			if(snakeLength <= MAX_SNAKE_SIZE) {
					add_to_score(3);
					return ATE_FOOD;
			} else {
		...
		return MOVE_OK; //no food at position
	}
}
...
*/
uint8_t set_snake_dirn(SnakeDirnType dirn) {
	...
	** next direction.
	*/
    if ((dirn == SNAKE_UP && curSnakeDirn == SNAKE_DOWN) || (dirn == SNAKE_DOWN && curSnakeDirn == SNAKE_UP) || (dirn == SNAKE_LEFT && curSnakeDirn == SNAKE_RIGHT) ||( dirn == SNAKE_RIGHT && curSnakeDirn == SNAKE_LEFT)) {
		return 0;
	} else {
		nextSnakeDirn = dirn;
		return 1;
