...
#define FOOD_COLOUR			COLOUR_YELLOW
...
void init_game(void) {
	...
	for(int8_t i = 0; i < 3; i++) {
		...
		if (i ==2) { //make last a rat
			rat_posn = add_food_item();
				if(is_position_valid(rat_posn)) {
					update_display_at_position(rat_posn, COLOUR_LIGHT_GREEN);
					store_rat_posn(rat_posn);
				}
		}
	}
}
...
int8_t attempt_to_move_snake_forward(void) {
	...
	if(move_result == ATE_FOOD || move_result == ATE_FOOD_BUT_CANT_GROW) {
		//RATS
		if (new_head_position == return_rat_posn()) { //if a rat is eaten
			remove_food(food_at(new_head_position)); //delete from array, no need for display
		...
		else if (return_stored_super() == new_head_position) {
			remove_food(food_at(new_head_position));
		} else {
		//NORMAL FOOD
